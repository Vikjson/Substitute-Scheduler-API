...
# Extra API tests for VG
echo
echo "Running extra API tests..."

# Test 1: curl test substitute_id in JSON
echo "Test 1: Valid substitute_id=1 in JSON"
curl -s "${END_POINT}?format=json&substitute_id=1" | jq '.' &> /dev/null || fail "Test 1 failed: invalid JSON for substitute_id=1"
echo Pass!

# Test 2: Invalid substitute_id=999 in XML
echo "Test 2: Invalid substitute_id=999 in XML"
curl -s "${END_POINT}?format=xml&substitute_id=999" | xmllint - &> /dev/null || fail "Test 2 failed: invalid XML for substitute_id=999"
curl -s "${END_POINT}?format=xml&substitute_id=999" | xmllint - | egrep '<schedules></schedules>|<schedules/>' &> /dev/null || fail 'Test 2 failed: Expected empty <schedules> element'
echo Pass!